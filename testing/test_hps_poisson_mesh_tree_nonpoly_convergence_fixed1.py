...
def global_relative_L2_errors(
  ...
  """Return HPS and best elementwise projection errors over the whole mesh.

  The best-approximation comparison is discontinuous across elements: on each
  element it is the weighted L2 projection of the exact solution into the same
  local polynomial space Pi_n^D. Both squared errors and the reference norm are
  accumulated over all physical elements before taking square roots.
  """
  ...
  for element_id, simplex in enumerate(simplices):
    ...
    # RefSimplexPrecomp's modal basis is orthonormal in the quadrature weight,
    # so these are the elementwise best weighted-L2 projection coefficients.
...
def save_results_csv(rows: list[dict[str, float | int]], path: Path) -> None:
  fieldnames = [
    ...
    "fallback_splits", "alpha", "beta", "tau_C",
    "q_solve_vol", "q_solve_face", "q_data_vol", "q_data_face", "q_eval",
    "root_residual", "interface_residual", "parent_residual",
    "relative_L2_error", "best_relative_L2_error", "hps_to_best_L2_ratio",
  ]
...
def plot_hps_vs_best_l2_vs_n(
  ...
  """Compare the HPS error with the mesh-wide best local L2 projection."""
  ...
  plt.xlabel(r"polynomial degree $n$")
  plt.ylabel(r"relative $L^2$ error")
  plt.title("HPS error versus best elementwise polynomial approximation")
  plt.xticks(sorted({int(row["n"]) for row in rows}))
  plt.grid(True, which="both", linewidth=0.5)
...
def main() -> None:
  ...
  if args.q_pad < 0 or args.q_data_pad < 0 or args.q_eval_pad < 0:
  ...
  for D in dimensions:
    ...
    for n in range(args.min_n, args.max_n + 1):
      ...
      q_data_vol = n + args.q_data_pad
      q_data_face = 1 if D == 1 else n + args.q_data_pad
  ...
  plot_best_path = args.output_dir / "hps_poisson_nonpoly_hps_vs_best_l2_vs_n.png"
