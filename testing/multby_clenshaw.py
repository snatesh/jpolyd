...
class MultByQClenshaw:
  ...
  This matches the square-L construction in construct_LD_general.py exactly:
    - same canonical pivot rule
    - same Aeff/Beff/Ceff extraction from J_poly (degree p)
    - same row-picked -(I ⊗ J_fun_i) operator using J_fun (degree K)
  """
  ...
  def __init__(self, D, p, K, kappa, assume_symmetric=True):
    ...
    # Precompute per-level effective blocks and sigma maps
    self.Aeff = [None] * self.p     # j=0..p-1, shape (m_{j+1} x m_j)
    self.Beff = [None] * self.p     # j=0..p-1, shape (m_{j+1} x m_{j+1})
    self.Ceff = [None] * self.p     # j=0..p-1, shape (m_{j+1} x m_{j-1}) for j>=1 else None
    ...
    # Dense factorizations of Beff^T for solves at each j
    # Needed for diagonal blocks (B_{j-1}^T ⊗ I) when solving for v_j (j>=1).
    self.BT_lu = [None] * self.p  # store lu_factor for Beff[j].T, used when solving v_{j+1}
  ...
  def _precompute_level(self, j):
    ...
    # Pre-factorize Beff[j].T for solving v_{j+1} at backsub step
    # Note: Beff[j] is size m_{j+1} x m_{j+1}
    Bd = Beff.toarray()
    self.BT_lu[j] = la.lu_factor(Bd.T)
  ...
  def _apply_AkronI_T(self, j, w):
    ...
    """
    MK = self.MK
    A = self.Aeff[j].toarray()  # (m_{j+1}, m_j)
    mj = A.shape[1]
  ...
  def _apply_JT(self, j, w):
    ...
    # Accumulate contributions per b
    # out[b] += J_fun[i] @ W[row]
  ...
  def _solve_BT_kronI(self, j_minus_1, s):
    """
    Solve (Beff[j_minus_1]^T ⊗ I) v = s.
    Here Beff[j_minus_1] is size m_j x m_j, where j = (j_minus_1 + 1).
    s is length m_j*MK. Output v same length.
    """
    MK = self.MK
    mj = s.size // MK
    S = s.reshape((mj, MK), order="C")
    lu, piv = self.BT_lu[j_minus_1]
    V = la.lu_solve((lu, piv), S)  # solves Beff^T V = S for multiple RHS
  ...
  def apply(self, q_coeffs_p, c_coeffs_K):
    ...
    # Back substitution storage
    v = [None] * (self.p + 3)
    v[self.p + 1] = np.zeros(0, dtype=np.float64)  # unused placeholder
    v[self.p + 2] = np.zeros(0, dtype=np.float64)  # unused placeholder

    # Compute v_p from (Beff[p-1]^T ⊗ I) v_p = r_p
    if self.p == 0:
      # Special case: p=0 => L is just I, so v0 = r0 = q0 ⊗ c_K = q0*c_K
      return (r[0]).copy()
    ...
    for j in range(self.p - 1, 0, -1):
      # s_j = r_j - (A_j ⊗ I - J_j)^T v_{j+1} - (C_{j+1}^T ⊗ I) v_{j+2}
      term_A = self._apply_AkronI_T(j, v[j + 1])     # (m_j*MK)
      term_J = self._apply_JT(j, v[j + 1])           # (m_j*MK)
      s = r[j] - (term_A - term_J)

      if j + 1 <= self.p - 1:
        # C_{j+1} exists for (j+1)>=1 always, and is used with v_{j+2}
        # Here we need (C_{j+1}^T ⊗ I) v_{j+2}, which comes from level (j+1).
        if (j + 1) >= 1:
          s -= self._apply_CkronI_T(j + 1, v[j + 2])

      # Solve diagonal: (Beff[j-1]^T ⊗ I) v_j = s
      v[j] = self._solve_BT_kronI(j - 1, s)

    # Compute v0: v0 = r0 - (A0 ⊗ I - J0)^T v1 - (C1^T ⊗ I) v2
    term_A0 = self._apply_AkronI_T(0, v[1])
    term_J0 = self._apply_JT(0, v[1])
    v0 = r[0] - (term_A0 - term_J0)

    if self.p >= 2:
      # subtract (C1^T ⊗ I) v2, where C1 corresponds to level 1
      v0 -= self._apply_CkronI_T(1, v[2])

    # v0 has size m0*MK = 1*MK, so it is exactly y_K
    return v0.copy()
...
def multbyq_apply_clenshaw(D, N, p, kappa, c_fN, q_cp, K=None):
  """
  Apply multiplication-by-q operator with de-aliasing:
    - lift to K (default N+p)
    - compute y_K = M_q^(K) c_fK via block back-sub
    - restrict to Pi_N

  Inputs:
    c_fN: coefficients of f in Pi_N
    q_cp: coefficients of q in Pi_p
  Returns:
    y_N: coefficients of Pi_N projection of q*f (de-aliased)
  """
  if K is None:
    K = N + p
...
def test_eq212_lifted_backsub(D, N, p, kappa, nquad, f_fun, q_fun, K=None, tol=5e-12, verbose=True):
  ...
  # Apply back-substitution in lifted space then restrict
  y_N = multbyq_apply_clenshaw(D, N, p, kappa, c_fN, q_cp, K=K)

  # Baseline: project product (q*f) on Pi_N
  def qf_fun(Xin):
...
if __name__ == "__main__":
  ...
  # Quick smoke tests mirroring construct_LD_general.py
  D = 2
